...
describe('streamRoutes — error messages', () => {
  it('returns a specific, contextual reason instead of a generic internal error', async () => {
    prismaMock.paymentStream.findMany.mockRejectedValue(
    ...
    expect(res.body.error).toMatch(/Failed to list streams/);
    expect(res.body.error).toMatch(/connection pool exhausted/);
  });

  it('does not leak sensitive material in the error message', async () => {
    const secret = 'SXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'; // 56-char fake stellar secret seed
    prismaMock.paymentStream.findUnique.mockRejectedValue(
      new Error(`connection failed using ${secret} for node`),
    );
  ...
  it('preserves the input-validation mapping for non-numeric stream ids', async () => {
    prismaMock.paymentStream.findUnique.mockRejectedValue(
      new Error('Cannot convert 42 to BigInt'),
    );

    const res = await request(app)
      .get('/api/v1/streams/not-a-number')
      .set('x-wallet-address', 'user123');

    expect(res.status).toBe(400);
    expect(res.body.error).toBe('Invalid stream id');
  });
});
